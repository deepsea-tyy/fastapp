...
class AppWebSocket {
  ...
  /// 推送消息结构 数据为一层 {"event":"kefu_message","type":"push_message","action":"kefu_message", ... }
  ...
  void _handleMessage(dynamic rawMessage) {
    try {
      ...
      final action = message['data']['event'] as String?;
      if (action == 'kefu_message') {
        final data = message['data'];
        if (data != null) {
          _emitMessage(WebSocketMessageType.customerServiceMessage, '', data);
        }
        return;
      } else if (action == 'kefu_message_end') {
        final data = message['data'] ?? {};
        _emitMessage(WebSocketMessageType.customerServiceMessageEnd, '', data);
        return;
      }
  ...
  void sendCustomerServiceMessage({
    ...
    _sendMessage('kefu_message_send', {
  ...
  void markCustomerServiceMessageRead({
    required int conversationId,
    List<int>? messageIds,
  }) {
    _sendMessage('kefu_message_read', {
      'conversation_id': conversationId,
  ...
  void endCustomerServiceConversation(int conversationId) {
    _sendMessage('kefu_message_end', {
      'conversation_id': conversationId,
    });
  }
