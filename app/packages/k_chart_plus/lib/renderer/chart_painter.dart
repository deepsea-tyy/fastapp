...
class ChartPainter extends BaseChartPainter {
  ...
  final List<BaseChartRenderer> mSecondaryRendererList = [];
  ...
  ChartPainter(
    ...
    this.maDayList = const [5, 10, 20],
            ...
            volInsertPosition: volInsertPosition,
  ...
  void initChartRenderer() {
    ...
    final unifiedList = unifiedSecondaryList;
    for (int i = 0; i < mSecondaryRectList.length && i < unifiedList.length; i++) {
      final state = unifiedList[i];
  ...
  void drawBg(Canvas canvas, Size size) {
    final bgPaint = Paint()..color = chartColors.bgColor;
    ...
    final dateRectTop = size.height - mBottomPadding;
    canvas.drawRect(
      Rect.fromLTRB(0, dateRectTop, size.width, size.height),
      bgPaint,
    );
  }
  ...
  void drawVerticalText(canvas) {
    ...
    for (final renderer in mSecondaryRendererList) {
      renderer.drawVerticalText(canvas, textStyle, mGridRows);
    }
  }
  ...
  void drawDate(Canvas canvas, Size size) {
    ...
    double x = 0.0;
    double y = 0.0;
    final dateYBase = size.height - mBottomPadding;
    ...
    for (var i = 0; i <= mGridColumns; ++i) {
      ...
      if (translateX >= startX && translateX <= stopX) {
        ...
        y = dateYBase + (mBottomPadding - tp.height) / 2;
        if (y + tp.height > size.height) {
          y = size.height - tp.height;
        }
  ...
  void drawCrossLineText(Canvas canvas, Size size) {
    ...
    final dateYBase = size.height - mBottomPadding;
    y = dateYBase + mBottomPadding / 2 - textHeight / 2;
    if (y + textHeight > size.height) {
  ...
  void drawTrendLines(Canvas canvas, Size size) {
    ...
    trendLineX = x;
    double y = selectY;
    final lineBottom = size.height - mBottomPadding;
    canvas.drawLine(Offset(x, mTopPadding),
        Offset(x, lineBottom), paintY);
    Paint paintX = Paint()
      ..color = chartColors.trendLineColor
      ..strokeWidth = 1
      ..isAntiAlias = true;
