...
class MainRenderer extends BaseChartRenderer<CandleEntity> {
  ...
  void drawMaLine(CandleEntity lastPoint, CandleEntity curPoint, Canvas canvas,
    ...
    final maListLength = curPoint.maValueList?.length ?? 0;
    if (maListLength == 0) return;
    
    for (int i = 0; i < maListLength && i < 3; i++) {
      final lastMA = lastPoint.maValueList?[i] ?? 0;
      final curMA = curPoint.maValueList?[i] ?? 0;
      
      if (lastMA > 0 || curMA > 0) {
        final startMA = lastMA > 0 ? lastMA : curMA;
        final endMA = curMA > 0 ? curMA : startMA;
        final maColor = this.chartColors.getMAColor(i);
        
        final startY = getY(startMA);
        final endY = getY(endMA);
        final paint = Paint()
          ..color = maColor
          ..strokeWidth = 2.0
          ..style = PaintingStyle.stroke
        ...
        canvas.drawLine(Offset(lastX, startY), Offset(curX, endY), paint);
      }
    }
  }
