...
```bash
# 生成CRUD代码
php bin/hyperf.php ds:crud --module=admin --table=users

# 强制覆盖
php bin/hyperf.php ds:crud --module=admin --table=users --force=true

# 插件模式
php bin/hyperf.php ds:crud --module=admin --plugin=ds/cms --table=articles

# API模式（只生成PHP代码）
php bin/hyperf.php ds:crud --module=api --table=users --target=api

# 自动执行SQL
php bin/hyperf.php ds:crud --module=admin --table=users --sql=true
```
...
**中间件差异**：
- **admin模式**：
  - 类级别：`AccessTokenMiddleware` + `PermissionMiddleware`
  - 方法级别：`OperationMiddleware`（仅在新增、修改、删除方法上使用，用于记录操作日志）
- **api模式**：`TokenMiddleware`（JWT Token 验证，方法级别配置）
...
> **注意**：前端文件路径由 `config/autoload/plugin.php` 中的 `front_directory` 配置决定，默认为项目根目录下的 `admin/` 目录。

```
{front_directory}/src/modules/{module}/
├── api/                                    # API接口定义
...
**默认路径**（相对于项目根目录）：
- `web/src/modules/{module}/` - 默认前端目录

**配置说明**：
- 前端目录路径在 `config/autoload/plugin.php` 中配置
- 支持绝对路径和相对路径（相对于 `BASE_PATH`）
- 代码生成器会自动使用配置的前端目录路径
...
集成操作日志中间件（`OperationMiddleware`），自动记录新增、修改、删除操作。

**配置说明**：
- `OperationMiddleware` 仅在新增（`create`）、修改（`save`）、删除（`delete`）方法上使用
- 查询方法（如 `pageList`、`list`）不使用操作日志中间件
- 自动记录操作人、操作路径、IP地址等信息
