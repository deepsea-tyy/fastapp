...
# 示例：创建内容管理插件
php bin/hyperf.php plugin:create ds/cms --name="内容管理" --type=mix --description="内容发布和管理功能" --author="ds"
```
...
```bash
# 安装本地插件
php bin/hyperf.php plugin:install ds/cms

# 静默安装（无需确认）
php bin/hyperf.php plugin:install ds/cms --yes
```
...
```bash
# 卸载插件
php bin/hyperf.php plugin:uninstall ds/cms

# 静默卸载
php bin/hyperf.php plugin:uninstall ds/cms --yes
```
...
```bash
# 显示插件可发布的配置文件
php bin/hyperf.php plugin:script ds/cms --show

# 发布插件配置文件
php bin/hyperf.php plugin:script ds/cms --id=<配置ID>

# 强制覆盖发布
php bin/hyperf.php plugin:script ds/cms --id=<配置ID> --force
```
...
- **组织名**：使用小写字母，如 `ds`
- **插件名**：使用小写字母和连字符，如 `cms` 或 `cms-manager`
- **命名空间**：`Plugin\\组织名\\插件名\\`
