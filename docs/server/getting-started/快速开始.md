...
# 方式3：后台运行（推荐生产环境）
nohup php -d swoole.use_shortname='Off' bin/hyperf.php start > runtime/hyperf.log 2>&1 &
```

> 💡 **提示**：
> - 前台运行：便于开发调试，按 `Ctrl+C` 停止服务
> - 后台运行：使用 `nohup` 或配置 `daemonize` 选项，适合生产环境
> - 停止服务：使用 `./stop.sh` 脚本或 `kill` 命令停止进程
...
# 数据库迁移
php bin/hyperf.php migrate                    # 运行所有迁移
php bin/hyperf.php migrate:rollback          # 回滚最后一次迁移
php bin/hyperf.php migrate:reset             # 回滚所有迁移
php bin/hyperf.php migrate:refresh           # 重置并重新运行所有迁移
php bin/hyperf.php migrate:status            # 查看迁移状态

# 数据填充
php bin/hyperf.php db:seed                   # 运行 databases/seeders 下全部 Seeder
php bin/hyperf.php db:seed --path=相对路径   # 仅执行指定 Seeder 文件或目录（不合并全局 seeders）
...
# 初始化JWT密钥（首次使用）
php bin/hyperf.php jwt:secret                # 生成十六进制密钥（默认）
php bin/hyperf.php jwt:secret --base64       # 生成 Base64 编码的密钥
php bin/hyperf.php jwt:secret --force        # 强制更新已存在的密钥
php bin/hyperf.php jwt:secret --length=64    # 指定密钥长度（字节数，默认32）
```
