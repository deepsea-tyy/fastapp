...
```typescript
// plugin/ds/cms/web/api/article.ts
...
**使用**：`import { page } from '$/ds/cms/api/article.ts'`

### 路径别名

- `$/`：插件根目录（`$/ds/cms/api/article.ts`）
- `@/`：前端项目根目录
- `~/`：前端 src 目录
...
```bash
php bin/hyperf.php plugin:install ds/cms    # 安装
php bin/hyperf.php plugin:uninstall ds/cms # 卸载
```
...
| 如何引用插件API | 使用路径别名：`import { page } from '$/ds/cms/api/article.ts'` |
