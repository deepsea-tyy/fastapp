  ...
  setup() {
    ...
    const colorModeSettings = () => {
      return tabsRender(
        'colorMode',
        [
          { icon: 'material-symbols:sunny-outline-rounded', label: useTrans('base.settings.colorModes.light') as string, value: 'light' },
          { icon: 'material-symbols:dark-mode-outline', label: useTrans('base.settings.colorModes.dark') as string, value: 'dark' },
          { icon: 'lets-icons:color-mode-light', label: useTrans('base.settings.colorModes.system') as string, value: 'autoMode' },
        ],
    ...
    const layoutsRender = () => {
      return (
        <div class="mine-layout-setting">
          <m-tooltip text={useTrans('base.settings.layouts.classic')}>
            <div
          ...
          <m-tooltip text={useTrans('base.settings.layouts.columns')}>
            <div
          ...
          <m-tooltip text={useTrans('base.settings.layouts.mixed')}>
            <div
        ...
        <>
          {
            settingsRender('tabbar', [
              { label: useTrans('base.settings.tabbars.enable') as string, value: 'enable' },
            ])
          }
          <div class="mine-setting-description">
            <div class="desc-label">{useTrans('base.settings.tabbars.mode')}</div>
            <div class="desc-value">
              {tabsRender('tabMode', [
                { label: useTrans('base.settings.tabbars.modeDefault') as string, value: 'rectangle' },
                { label: useTrans('base.settings.tabbars.modeCard') as string, value: 'card' },
      ...
      return (
        <>
          ...
          <div class="mt-3 flex items-center justify-between rounded p-2 ring-2 ring-[rgb(var(--ui-primary))]">
            <div class="text-sm">{useTrans('base.toolbars.sorts')}</div>
    ...
    const copyrightRender = () => {
      ...
      const options = [
        { label: useTrans('base.settings.copyRights.date') as string, value: 'dates' },
        { label: useTrans('base.settings.copyRights.company') as string, value: 'company' },
        { label: useTrans('base.settings.copyRights.website') as string, value: 'website' },
        { label: useTrans('base.settings.copyRights.putOnRecord') as string, value: 'putOnRecord' },
      ]
      return (
        <>
          {
            settingsRender('copyright', [
              { label: useTrans('base.settings.copyRights.enable') as string, value: 'enable' },
            ])
          }
          { options.map((item: any) => (
            <div class="mine-setting-description">
    ...
    return () => (
      <div class="flex items-center">
        <m-drawer
          ...
          title={useTrans('base.settings.title')}
          v-slots={{
            trigger: () => (
              <ma-svg-icon
            ...
            default: () => (
              <div class="pb-10">
                ...
                {keys.value.colorMode !== 'dark' && settingsRender('app', [
                  { label: useTrans('base.settings.asideDark') as string,
                    value: 'asideDark',
                    change: (v: boolean) => settingStore.setAsideDark(v),
                  },
                ])}

                {divider(useTrans('base.settings.primaryColorSetting') as string)}
                <ul class="mine-setting-color-list">
                  {colorListRender()}
                </ul>

                {divider(useTrans('base.settings.layoutSetting') as string)}
                {layoutsRender()}

                {divider(useTrans('base.settings.mainAsideSetting') as string)}
                {
                  settingsRender('mainAside', [
                    { label: useTrans('base.settings.mainAsides.mainMenuTitle') as string, value: 'showTitle' },
                    { label: useTrans('base.settings.mainAsides.autoToFirstMenu') as string, value: 'enableOpenFirstRoute' },
                  ])
                }

                {divider(useTrans('base.settings.tabBarSettings') as string)}
                {tabSettingRender()}

                {divider(useTrans('base.settings.toolBarSettings') as string)}
                {toolbarHook.state && toolbarRender()}

                {divider(useTrans('base.settings.copyRightSettings') as string)}
                {copyrightRender()}

                {divider(useTrans('base.settings.otherSettings') as string)}
                {
                  settingsRender('app', [
                ...
                <div class="mine-setting-description">
                  <div class="desc-label !w-6/12">{useTrans('base.settings.watermarkText')}</div>
                  <div class="desc-value !w-6/12">
                    <m-input
                      class="mb-1 !h-6 !py-1"
