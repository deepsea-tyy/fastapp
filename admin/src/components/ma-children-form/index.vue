...
// 渲染字段的组件
const RenderConfigFields = defineComponent({
  props: {
    rowData: {
      type: Object,
      required: true
  ...
  setup(componentProps) {
    return () => {
      return props.formList.map((item: ChildFormItem) => {
        const component = getComponentByFormType(item.formType)
        const componentPropsConfig = getComponentProps(item)
        ...
        // 当前字段的值 - 直接访问响应式数据
        const currentValue = componentProps.rowData[item.dataIndex]

        // 控件通用属性
        const controlPropsBase = {
          placeholder: componentPropsConfig.placeholder,
          clearable: true,
        ...
        if (item.formType === 'select' && item.dict && item.dict.data) {
          ...
          content = h(ElSelect, {
            ...
            'onUpdate:modelValue': (val: any) => {
              updateItemField(componentProps.rowIndex, item.dataIndex, val)
            }
          }, () => options.map(opt =>
            h(ElOption, {
              key: opt.value,
          ...
          // 渲染富文本编辑器
          content = h(MaEditor, {
            height: componentPropsConfig.height || 400,
            language: componentPropsConfig.language || 'zh_CN',
            toolbar: componentPropsConfig.toolbar,
            modelValue: currentValue,
            'onUpdate:modelValue': (val: any) => {
              updateItemField(componentProps.rowIndex, item.dataIndex, val)
            }
        ...
        } else if (component === 'el-input' && componentPropsConfig.type === 'textarea') {
          // 渲染文本域
          content = h(ElInput, {
            ...controlPropsBase,
            type: 'textarea',
            autosize: componentPropsConfig.autosize,
            modelValue: currentValue,
            'onUpdate:modelValue': (val: any) => {
              updateItemField(componentProps.rowIndex, item.dataIndex, val)
            }
          ...
          content = h(ElInput, {
            ...controlPropsBase,
            type: componentPropsConfig.type,
            autosize: componentPropsConfig.autosize,
            modelValue: currentValue,
            'onUpdate:modelValue': (val: any) => {
              updateItemField(componentProps.rowIndex, item.dataIndex, val)
        ...
        // 返回完整的字段
        return h('div', {
          key: `${componentProps.rowIndex}-${item.dataIndex}`,
      ...
      <div class="ma-children-form-item-content">
        ...
        <template v-else-if="props.formList && props.formList.length > 0">
          <div class="ma-children-form-fields">
            <render-config-fields :row-data="item" :row-index="index" />
          </div>
        </template>
      </div>
    </div>
