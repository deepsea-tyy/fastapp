...
export function getRoleDepartments(id: number): Promise<ResponseStruct<Array<{ id: number; name: string }>>> {
  return useHttp().get(`/admin/role/${id}/departments`)
}

export function setRoleDepartments(id: number, departments: number[]): Promise<ResponseStruct<null>> {
  return useHttp().put(`/admin/role/${id}/departments`, { departments })
}
