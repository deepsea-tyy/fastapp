...
import type useWebSocketStore from '@/store/modules/useWebSocketStore'
 ...
 * 会话列表管理 Hook
 *
 * @param wsStore - WebSocket Store 实例
 * @param t - 国际化函数
 * @returns 会话管理接口
 */
export function useConversations(
  ...
  function updateConversationUnread(conversationId: number, unreadCount: number) {
    ...
    // 重新计算管理员的未读总数 - 直接从 treeData 中查找以避免缓存冲突
    const kefuNode = treeData.value.find((node) => node.kefu_id === conversation.kefu_id)
    if (kefuNode) {
  ...
  function handleWebSocketConversationUpdate(message: WebSocketMessage) {
    ...
    if (!conversation && kefuId) {
      ...
      if (!isNaN(kefuIdNum)) {
        const kefuNode = treeData.value.find((node) => node.kefu_id === kefuIdNum)
        if (kefuNode) {
          // 刷新会话列表以获取最新信息
          loadConversations()
          return
