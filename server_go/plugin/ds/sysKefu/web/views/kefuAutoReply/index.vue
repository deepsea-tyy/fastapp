...
import { deleteByIds, page } from '$/ds/syskefu/api/kefuAutoReply.ts'
...
defineOptions({ name: 'ds:syskefu:kefu_auto_reply' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('admin.KefuAutoReply'),
  },
  // 表格参数
  tableOptions: {
    // 表格树
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['ds:syskefu:kefu_auto_reply:create']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['ds:syskefu:kefu_auto_reply:delete']"
          type="danger"
          plain
