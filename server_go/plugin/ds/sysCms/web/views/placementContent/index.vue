...
import { deleteByIds, page } from '$/ds/sysCms/api/placementContent.ts'
...
defineOptions({ name: 'ds:sysCms:placement_content' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('admin.PlacementContent'),
  },
  // 表格参数
  tableOptions: {
    // 表格树
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['ds:sysCms:placement_content:create']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['ds:sysCms:placement_content:delete']"
          type="danger"
          plain
