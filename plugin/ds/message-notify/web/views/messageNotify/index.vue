...
import { deleteByIds, page } from '$/ds/message-notify/api/messageNotify.ts'
...
defineOptions({ name: 'ds:message-notify:message_notify' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t("admin.MessageNotify"),
  },
  // 表格参数
  tableOptions: {
    // 表格树
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['ds:message-notify:message_notify:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['ds:message-notify:message_notify:delete']"
          type="danger"
          plain
