...
export default function useChatLogic() {
  ...
  // 1. 使用全局 WebSocket Store
  const wsStore = useWebSocketStore()

  // 2. 初始化会话管理模块（依赖 WebSocket Store）
  const conversationsModule = useConversations(wsStore, t)

  // 3. 初始化消息管理模块（依赖会话管理）
  const messagesModule = useMessages(conversationsModule, msg)

  // 4. 注册 WebSocket 消息处理器
  wsStore.registerMessageHandler(
    WS_ACTION.PUSH_USER_MESSAGE,
    conversationsModule.handleWebSocketConversationUpdate,
  )

  wsStore.registerMessageHandler(
    WS_ACTION.PUSH_VISITOR_MESSAGE,
    (message: any) => {
      ...
      if (conversationsModule.selectedConversation.value &&
        ...
        if (message.sender_type === CHAT_CONSTANTS.SENDER_TYPE.USER) {
          ...
          if (conversation) {
            ...
            // 重新计算管理员未读数 - 直接从 treeData 中查找
            const kefuNode = conversationsModule.treeData.find(
              (node) => node.kefu_id === conversation.kefu_id,
            )
            if (kefuNode) {
  ...
  function cleanupMessageHandlers() {
    wsStore.unregisterMessageHandler(WS_ACTION.PUSH_USER_MESSAGE)
    wsStore.unregisterMessageHandler(WS_ACTION.PUSH_VISITOR_MESSAGE)
    wsStore.unregisterMessageHandler(WS_ACTION.PUSH_VISITOR_END)
  }
