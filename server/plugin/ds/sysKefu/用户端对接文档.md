...
客服插件（ds/sysKefu）提供完整的客服功能，用户端需要对接以下内容：
...
**接口地址**：`GET /api/sysKefu/message/getConversation`
...
**请求示例**：
```javascript
const response = await fetch('/api/sysKefu/message/getConversation', {
  headers: {
...
**接口地址**：`GET /api/sysKefu/message/list`
...
const response = await fetch(
  `/api/sysKefu/message/list?conversation_id=${conversationId}&page=${page}&page_size=${pageSize}`,
  {
    headers: {
      'Authorization': 'Bearer your_user_token'
    }
...
4. **连接绑定**
   - 建议在建立连接后立即调用 `visitor.bind_fd` 绑定连接
   - 绑定后，客服发送的消息会推送到该连接
...
**请求示例**：
```javascript
GET /api/sysKefu/message/getConversation
Headers: {
  'Authorization': 'Bearer your_user_token'
}
```
...
**请求示例**：
```javascript
GET /api/sysKefu/message/list?conversation_id=1&page=1&page_size=20
Headers: {
  'Authorization': 'Bearer your_user_token'
}
```
