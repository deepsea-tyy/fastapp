...
import { deleteByIds, page } from '$/ds/sysCms/api/placementPosition.ts'
...
defineOptions({ name: 'ds:sysCms:placement_position' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('admin.PlacementPosition'),
  },
  // 表格参数
  tableOptions: {
    // 表格树
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['ds:sysCms:placement_position:create']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['ds:sysCms:placement_position:delete']"
          type="danger"
          plain
