...
import { deleteByIds, page } from '$/ds/sysCms/api/appPageContent.ts'
...
defineOptions({ name: 'ds:sysCms:app_page_content' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('admin.AppPageContent'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['ds:sysCms:app_page_content:create']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['ds:sysCms:app_page_content:delete']"
          type="danger"
          plain
