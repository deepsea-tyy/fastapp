...
import type { MessageNotifyVo } from '$/ds/sysNotify/api/messageNotify.ts'
...
export default function getFormItems(formType: 'add' | 'edit' = 'add', t: any, model: MessageNotifyVo): MaFormItem[] {
  ...
  if (formType === 'add') {
    ...
    if (!model.title || !Array.isArray(model.title)) {
      model.title = [{ lang: 'zh_CN', text: '' }]
    }
    if (!model.content || !Array.isArray(model.content)) {
      model.content = [{ lang: 'zh_CN', text: '' }]
    }
  }
  ...
  if (formType === 'edit') {
    // 确保多语言字段是数组格式
    if (!model.title || !Array.isArray(model.title)) {
      model.title = [{lang: 'zh_CN', text: model.title}]
    }
    if (!model.content || !Array.isArray(model.content)) {
      model.content = [{lang: 'zh_CN', text: model.content}]
    }
  }
  ...
  return [
    ...
    },
    {
      ...
      renderProps: {
        ...
        dictName: 'sys-notify-notify-type',
        clearable: true,
      },
    },
    {
