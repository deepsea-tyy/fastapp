...
return [
    'default' => [
        'driver' => Jwt::class,
        'key' => getJwtKey(env('JWT_SECRET')),  // 自动检测 base64 或 plain text
        'alg' => new Sha256(),
    ...
    'api' => [
        ...
        'key' => getJwtKey(env('JWT_API_SECRET')),  // 自动检测 base64 或 plain text
        // ... 其他配置同上
    ],
];
```
...
```bash
# 生成 Base64 编码的密钥（默认）
php bin/hyperf.php jwt:init-secret

# 生成十六进制密钥
php bin/hyperf.php jwt:init-secret --base64=false

# 指定密钥长度（字节数，默认32）
php bin/hyperf.php jwt:init-secret --length=64

# 强制更新已存在的密钥
php bin/hyperf.php jwt:init-secret --force
```

**手动配置**：
- `JWT_SECRET` 和 `JWT_API_SECRET` 支持两种格式：
  - **Base64 编码**：使用 `base64_encode(random_bytes(32))` 生成
  - **Plain Text**：直接使用字符串（不推荐，安全性较低）
- 系统会自动检测密钥格式，无需手动指定

> 💡 **注意**：`getJwtKey()` 函数会自动检测密钥是 Base64 编码还是纯文本，并返回相应的 `InMemory` 实例。
