...
desktop_ensure_swoole_cli() {
  ...
  cli=$(desktop_swoole_cli_for_bundle "$bundle") || {
    echo "swoole-cli 不存在: $bundle" >&2
    return 1
  }

  DESKTOP_SWOOLE_BUNDLE=$bundle
  DESKTOP_SWOOLE_CLI=$cli
}
